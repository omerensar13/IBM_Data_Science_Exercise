...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        pie_fig_all = px.pie(filtered_df, values='class', 
                names='Launch Site', 
                title='Total Success Launches by Sites')
        return pie_fig_all
    else:
        filtered_df = spacex_df[spacex_df["Launch Site"] == str(entered_site)].groupby('class').count().reset_index()
        pie_fig_filtered = px.pie(filtered_df, values='Flight Number',
                        names='class', 
                        title=f'Total Success Launches for Site {entered_site}')
        return pie_fig_filtered
